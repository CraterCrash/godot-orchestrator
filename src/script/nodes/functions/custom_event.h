...
//
#ifndef ORCHESTRATOR_NODE_CUSTOM_EVENT_H
#define ORCHESTRATOR_NODE_CUSTOM_EVENT_H

#include "script/nodes/functions/function_entry.h"

using namespace godot;

/// Script node that represents an custom event handler
///
/// Should function like Unreal custom event nodes,
/// allowing execution to begin in arbitrary locations.
///
/// ** Work In Progress **
///
class OScriptNodeCustomEvent : public OScriptNodeFunctionEntry
{
    ORCHESTRATOR_NODE_CLASS(OScriptNodeCustomEvent, OScriptNodeFunctionEntry);
...
protected:
    ...
    bool _is_user_defined() const override { return true; }
    //~ End OScriptNodeFunctionEntry Interface

public:
    StringName event_name;

    OScriptNodeCustomEvent();
    ...
    static bool is_event_method(const MethodInfo& p_method) { return false; }
};

#endif  // ORCHESTRATOR_NODE_CUSTOM_EVENT_H