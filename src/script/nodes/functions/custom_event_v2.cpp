...
OScriptNodeCustomEvent_V2::OScriptNodeCustomEvent_V2()
{
    _flags = ScriptNodeFlags::CATALOGABLE;
}

String OScriptNodeCustomEvent_V2::get_tooltip_text() const
{
...
String OScriptNodeCustomEvent_V2::get_node_title() const
{
    if (_function.is_valid())
        return vformat("%s Custom Event", _function->get_function_name().capitalize());

    return super::get_node_title();
}

String OScriptNodeCustomEvent_V2::get_help_topic() const
{
...
StringName OScriptNodeCustomEvent_V2::resolve_type_class(const Ref<OScriptNodePin>& p_pin) const
{
    if (_function.is_valid())
    {
