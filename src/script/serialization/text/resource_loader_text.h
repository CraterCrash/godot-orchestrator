...
/// A text-based resource format loader for OScript script resources.
class OScriptTextResourceFormatLoader : public ResourceFormatLoader {
    GDCLASS(OScriptTextResourceFormatLoader, ResourceFormatLoader);

    friend class OScriptCache;

    static bool _is_text_script(const String& p_path);
