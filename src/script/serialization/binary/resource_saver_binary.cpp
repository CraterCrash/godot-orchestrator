...
bool OScriptBinaryResourceFormatSaver::_recognize_path(const Ref<Resource>& p_resource, const String& p_path) const {
    return p_path.get_extension() == ORCHESTRATOR_SCRIPT_EXTENSION;
}

bool OScriptBinaryResourceFormatSaver::_recognize(const Ref<Resource>& p_resource) const {
    return p_resource.is_valid() && p_resource->get_class() == OScript::get_class_static();
}

Error OScriptBinaryResourceFormatSaver::_set_uid(const String& p_path, int64_t p_uid) {
    OrchestrationBinarySerializer serializer;
    return serializer.set_uid(p_path, p_uid);
}

Error OScriptBinaryResourceFormatSaver::_save(const Ref<Resource>& p_resource, const String& p_path, uint32_t p_flags) {
...
void OScriptBinaryResourceFormatSaver::_bind_methods() {
}