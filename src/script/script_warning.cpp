...
// Taken from core/string/ustring.h
#define PNAME(m_value) (m_value)

String OScriptWarning::get_message() const {
	...
	ERR_FAIL_V_MSG(String(), vformat(R"(Invalid OScript warning "%s".)", get_name_from_code(code)));
    #undef CHECK_SYMBOLS
}

int OScriptWarning::get_default_value(Code p_code) {
...
PropertyInfo OScriptWarning::get_property_info(Code p_code) {
	return PropertyInfo(Variant::INT, get_setting_path_from_code(p_code), PROPERTY_HINT_ENUM, "Ignore,Warn,Error");
}

String OScriptWarning::get_name() const {
	return get_name_from_code(code);
}

String OScriptWarning::get_name_from_code(Code p_code) {
    ...
    return names[static_cast<int>(p_code)];
}

String OScriptWarning::get_setting_path_from_code(Code p_code) {
	return "debug/warnings/" + get_name_from_code(p_code).to_lower();
}

OScriptWarning::Code OScriptWarning::get_code_from_name(const String &p_name) {
