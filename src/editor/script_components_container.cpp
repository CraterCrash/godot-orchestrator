...
Orchestration* OrchestratorScriptComponentsContainer::_get_orchestration()
{
...
Variant OrchestratorScriptComponentsContainer::_component_item_dragged(TreeItem* p_item, const Vector2& p_position)
{
    ...
    ERR_FAIL_COND_V(!_script.is_valid(), Variant());
...
void OrchestratorScriptComponentsContainer::_component_item_button_clicked(Node* p_node, TreeItem* p_item, int p_column, int p_id, int p_button)
{
    ...
    switch (type)
    {
        ...
        case SCRIPT_FUNCTION:
        {
            const Vector<Node*> scene_nodes = SceneUtils::find_all_nodes_for_script_in_edited_scene(_script);
...
{
    ERR_FAIL_COND_MSG(!_script.is_valid(), "Cannot add component, script is invalid");
...
void OrchestratorScriptComponentsContainer::_component_add_item_commit(TreeItem* p_item)
{
    ...
    ERR_FAIL_COND_MSG(!_script.is_valid(), "Cannot add component item, script is invalid");
...
void OrchestratorScriptComponentsContainer::_component_rename_item(TreeItem* p_item)
{
    ...
    ERR_FAIL_COND_MSG(!_script.is_valid(), "Cannot rename component item, script is invalid");
...
void OrchestratorScriptComponentsContainer::_component_remove_item(TreeItem* p_item, bool p_confirm)
{
    ...
    ERR_FAIL_COND_MSG(!_script.is_valid(), "Cannot remove component item, script is invalid");
...
void OrchestratorScriptComponentsContainer::_component_focus_item(TreeItem* p_item)
{
    ...
    ERR_FAIL_COND_MSG(!_script.is_valid(), "Cannot focus component item, script is invalid");
...
{
    ERR_FAIL_COND_MSG(!_script.is_valid(), "Script is invalid");
...
void OrchestratorScriptComponentsContainer::_update_variables()
{
    ...
    for (const String& variable_name : variable_names)
    {
        ...
        if (variable->is_exported())
        {
            ...
            item->set_button_tooltip_text(0, index, "Variable is exported and visible out the script.");
        ...
        {
            String tooltip_text = "Variable is private and not exported.";
...
void OrchestratorScriptComponentsContainer::set_edited_resource(const Ref<Resource>& p_resource)
{
    ...
    // if (_script.is_valid())
    //     _orchestration = _script->get_orchestration();
}
