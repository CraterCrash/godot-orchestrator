...
void OrchestratorEditorSearchDialog::_favorite_activated() {
    _favorite_selected();
    _confirmed();
}

void OrchestratorEditorSearchDialog::_favorite_toggled() {
...
void OrchestratorEditorSearchDialog::_history_selected(int p_index) {
...
void OrchestratorEditorSearchDialog::_history_activated(int p_index) {
    _history_selected(p_index);
    _confirmed();
}
...
void OrchestratorEditorSearchDialog::_canceled() {
    emit_signal("closed");
    _cleanup();
}

void OrchestratorEditorSearchDialog::_item_selected() {
    TreeItem* item = _search_options->get_selected();
    if (!item) {
        return;
    }
...
void OrchestratorEditorSearchDialog::_filter_selected(int p_index) {
...
void OrchestratorEditorSearchDialog::popup_create(bool p_dont_clear, bool p_replace_mode, const String& p_current_type, const String& p_current_name) {
    ...
    Rect2 saved_size = PROJECT_GET("dialog_bounds", "create_new_node", Rect2());
    if (saved_size != Rect2()) {
        popup(saved_size);
    } else {
