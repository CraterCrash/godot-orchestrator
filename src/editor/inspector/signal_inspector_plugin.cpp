...
bool OrchestratorEditorInspectorPluginSignal::_can_handle(Object* p_object) const {
    return cast_to<OScriptSignal>(p_object) != nullptr;
}

bool OrchestratorEditorInspectorPluginSignal::_parse_property(Object* p_object, Variant::Type p_type, const String& p_name,
    PropertyHint p_hint_type, const String& p_hint_string, BitField<PropertyUsageFlags> p_usage_flags, bool p_wide)
{
    Ref<OScriptSignal> signal = cast_to<OScriptSignal>(p_object);
    if (!signal.is_valid()) {
        return false;
    }
    ...
    if (p_name.match("inputs")) {
        ...
        inputs->setup(true);
        inputs->connect("move_up", callable_mp_this(_move_up).bind(signal));
        inputs->connect("move_down", callable_mp_this(_move_down).bind(signal));
