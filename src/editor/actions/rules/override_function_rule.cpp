...
bool OrchestratorEditorActionOverrideFunctionRule::matches(const Ref<OrchestratorEditorActionDefinition>& p_action, const FilterContext& p_context) {
    ...
    if (p_action->method.has_value()) {
        ...
        if (method.flags & METHOD_FLAG_VIRTUAL) {
            if (_overridden_method_names.has(method.name)) {
                return false;
            }
