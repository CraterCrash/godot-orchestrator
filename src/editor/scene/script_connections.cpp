...
void OrchestratorScriptConnectionsDialog::_confirmed() {
    queue_free();
}
...
void OrchestratorScriptConnectionsDialog::popup_connections(const String& p_method, const Vector<Node*>& p_nodes) {
    ...
    for (int i = 0; i < p_nodes.size(); i++) {
        ...
        for (int j = 0; j < incoming_connections.size(); ++j) {
            ...
            Node* source = cast_to<Node>(ObjectDB::get_instance(signal.get_object_id()));
            ...
            Node* callable_node = cast_to<Node>(ObjectDB::get_instance(callable.get_object_id()));
