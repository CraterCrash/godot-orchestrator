...
// FBlueprintEditor::OnCollapseSelectionToFunction()
//      Gathers nodes & if collapsable, calls CollapseSelectionToFunction
//      Returns the UEdGraph for the function as the output
//
// FBlueprintEditor::CollapseSelectionToFunction
//      Finds new unique name for NewFunction
//      Calls FBlueprintEditorUtils::CreateNewGraph with name
//      Calls FBlueprintEditorUtils::AddFunctionGraph with the new graph
//      Gets the Entry node from the graph
//      Uses a ResultNodeCreator to create the FunctionResult
//      Repositions the result relative to the entry node
//      Uses UBlueprintFunctionNodeSpawner::Create and returns the OutFunctionNode
//      Then calls FBlueprintEditor::CollapseNodesIntoGraph(OutFunctionNode, EntryNode, ResultNode, SourceGraph, NewGraph, List of Nodes, true, false)
//      Lastly reconstructs the OutFunctionNode
//      Returns the NewGraph from step 2
//
// FBlueprintEditor::CollapseNodesIntoGraph
//      Iterates list of nodes
//      Tracks the Sum/Min/Max of Node X/Y positions
//      Removes node from source graph
//      Adds node to target graph
//      Then traverses pins for a node checking for cross-graph links
//
// Logic like this, can easily be moved to an OrchestrationEditor class that operates on the raw data model.
//
// The nice thing is that BlueprintEditor::GetSelectedNodes() is a helper that takes the current focused
// SGraphEditor object and gets the FGraphPanelSelectionSet.
//
// One performance improvement to consider would be to have OrchestratorEditorGraphPanel maintain a list
// of selected OrchestratorEditorGraphNode objects, so that getting the selection does not require a
// traversal everytime.
//
// FGraphPanelSelectionSet is simply a TSet<UObject*>
// SelectedItemType that is passed in is simply a UObject*
// What SNodePanel does is when you click on a node it uses a generic function called GetObjectBeingDisplayed()
//
