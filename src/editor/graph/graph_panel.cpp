...
void OrchestratorEditorGraphPanel::_show_node_context_menu(OrchestratorEditorGraphNode* p_node, const Vector2& p_position) {
    ...
    if (!are_multiple_selections) {
        menu->add_icon_shortcut(
            ...
            callable_mp(_markers, &OrchestratorEditorGraphMarkers::toggle_bookmark).bind(p_node));
    }
    ...
    if (!are_multiple_selections) {
        ...
        menu->add_shortcut(
            ...
            callable_mp(_markers, &OrchestratorEditorGraphMarkers::toggle_breakpoint).bind(p_node),
            false);
...
void OrchestratorEditorGraphPanel::_gui_input(const Ref<InputEvent>& p_event) {
    ...
    if (key.is_valid() && key->is_pressed()) {
        ...
        if (ED_IS_SHORTCUT("orchestrator_graph_editor/add_knot", p_event) && !_hovered_connection.is_empty()) {
