...
Vector2 OScriptNodePin::_calculate_midpoint_between_nodes(const Ref<OScriptNode>& p_source, const Ref<OScriptNode>& p_target) const {
    ...
    const Vector2 mid_point = Vector2(mid_x, mid_y) - (avg_size / 2); // NOLINT
...
void OScriptNodePin::set_type_override(const PropertyInfo& p_override) {
    ...
    _override_flags = 0;
    if (PropertyUtils::is_enum(p_override)) {
        _override_flags.set_flag(ENUM);
    } else if (PropertyUtils::is_bitfield(p_override)) {
        _override_flags.set_flag(BITFIELD);
    }
    if (p_override.hint == PROPERTY_HINT_FILE) {
        _override_flags.set_flag(FILE);
    } else if (p_override.hint == PROPERTY_HINT_MULTILINE_TEXT) {
        _override_flags.set_flag(MULTILINE);
    }
...
PackedStringArray OScriptNodePin::resolve_signal_names(bool p_self_fallback) const {
